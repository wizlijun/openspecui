...
Taichi Forge 是一个 **疯狂的** macOS 原生桌面应用，它把 AI 智能体扔进真实的终端会话里，让它们在你的代码库里疯狂输出——而 **你** 握着缰绳。
...
确认卡片中的修复操作统一通过 Fix 按钮路径发送（填充 textarea → 触发 Fix 按钮 → promptTemplate 包装 → Send 按钮发送），与 Self-Review Cycle 的调用逻辑完全一致，确保大段文字正确输入到 Droid 命令行。
...
大多数 AI 编程工具把你当乘客。Taichi Forge 把你当 **将军**。
